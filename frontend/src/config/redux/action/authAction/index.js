...
export const switchAccountAction = createAsyncThunk(
    ...
    async ({ userId }, thunkApi) => {
        try {
            // clientServer's default timeout is already cold-start-tolerant
            // (see REFRESH_TIMEOUT_MS in @/config) — this call single-use-
            // rotates a refresh cookie server-side regardless of whether the
            // client is still waiting, so a client that gave up too early
            // used to leave the OLD cookie permanently stale (confirmed via
            // backend track() logs: session_evicted_or_not_found with a
            // sessionCount well under the 10-session cap, ruling out
            // eviction as the cause).
